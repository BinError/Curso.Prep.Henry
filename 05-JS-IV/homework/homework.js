...
function pasarUsuarioAPremium(usuarios) {
  ...
  for(cadaUsuario in usuarios){
    usuarios[cadaUsuario].esPremium = true;
  }
  return usuarios;
}
...
function sumarLikesDeUsuario(usuario) {
  ...
  //likesTotal = usuario.posts[0].likes
  for(indices in usuario.posts){
    likesTotal += usuario.posts[indices].likes;
  }
  return likesTotal;
}
...
function agregarMetodoCalculoDescuento(producto) {
  ...
  producto.calcularPrecioDescuento = function(){
    var miDescuento = this.precio * this.porcentajeDeDescuento;
